...
function startGame() {
    ...
    // フェードイン用クラスを削除する
    const results = [
        document.getElementById('player-result'),
        document.getElementById('computer-result'),
        document.getElementById('winner-result')
    ];

    results.forEach(result => {
        result.classList.remove('fadein-up', 'fadein-down');
    });
}
...
function renderHands() {
    players.forEach(player => {
        ...
        player.hand.forEach((card, index) => {
            ...
            cardElement.style.animationDelay = '0.12s';
...
function determineWinner() {
    ...
    players.forEach(player => {
        ...
        resultElement.innerText = `${rankName}`;
        
        // 既存のアニメーションクラスを削除
        resultElement.classList.remove('fadein-up', 'fadein-down');

        // アニメーションクラスを追加
        if (player.isHuman) {
            resultElement.classList.add('fadein-up');
        } else {
            resultElement.classList.add('fadein-down');
        }
    });
